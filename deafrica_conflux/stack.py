...
def stack_polygon_timeseries_to_csv(
    ...
    polygons_stringids_to_tileids: dict[str, list[str]],
    ...
    for poly_uid in polygon_uids:
        ...
        filtered_drill_output_files = [
            ...
            if any(
                tileid in drill_output_file for tileid in polygons_stringids_to_tileids[poly_uid]
            )
        ]
        ...
        output_file_parent_directory = os.path.join(output_directory, f"{poly_uid[:4]}")
        output_file_path = os.path.join(output_file_parent_directory, f"{poly_uid}.csv")
